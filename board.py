...
class Board:
    ...
    def get_pos_color(self, rect):
        ...
        for i in range(4):
            ...
            if (rect.x + self.IMG_PLACE == 300 and
                rect.y + self.IMG_PLACE > 76 and rect.y + self.IMG_PLACE < 300):
                return self.BLUE
            if (rect.x + self.IMG_PLACE == 300 and
                rect.y + self.IMG_PLACE > 300 and rect.y + self.IMG_PLACE < 524):
                return self.GREEN
            if (rect.y + self.IMG_PLACE == 300 and
                rect.x + self.IMG_PLACE > 76 and rect.x + self.IMG_PLACE < 300):
                return self.YELLOW
            if (rect.y + self.IMG_PLACE == 300 and
                rect.x + self.IMG_PLACE > 300 and rect.x + self.IMG_PLACE < 524):
                return self.RED
        return self.WHITE
    ...
    def draw_board(self):
        ...
        for i in range(4):
            ...
            for j in range(4):
                pygame.draw.circle(self.screen, self.WHITE, (self.OUT[i][0] + self.PLACES[j][0], self.OUT[i][1] + self.PLACES[j][1]), 15, 0)
                pygame.draw.circle(self.screen, self.BLACK, (self.OUT[i][0] + self.PLACES[j][0], self.OUT[i][1] + self.PLACES[j][1]), 15, 1)

        self.draw_triangles(self.BLUE, (self.TOP_LEFT, self.TOP_RIGHT, self.CENTER))
        self.draw_triangles(self.RED, (self.TOP_RIGHT, self.BOTTOM_RIGHT, self.CENTER))
        self.draw_triangles(self.GREEN, (self.BOTTOM_RIGHT, self.BOTTOM_LEFT, self.CENTER))
        self.draw_triangles(self.YELLOW, (self.BOTTOM_LEFT, self.TOP_LEFT, self.CENTER))
        ...
        for i in range(2):
            for j in range(2):
                pygame.draw.circle(self.screen, self.WHITE, (236 + i*128, 236 + j*128), 15, 0)
                pygame.draw.circle(self.screen, self.BLACK, (236 + i*128, 236 + j*128), 15, 1)
    ...
    def draw_circles(self, color, begin_x, begin_y, direction_x, direction_y, count_x, count_y, last):
        for i in range(count_y):
            for j in range(count_x):
                ...
                pygame.draw.circle(self.screen, self.BLACK, (begin_x + direction_x*32*j, begin_y + direction_y*32*i), 15, 1)
