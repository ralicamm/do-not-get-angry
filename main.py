...
class Main:
    ...
    def __init__(self, size):
        ...
        self.board.add_label(595, 130, self.player.get_color_name(), 20, self.board.WHITE)
    ...
    def main(self):
        while self.running:
            ...
            """Add game turn label"""
            self.board.add_label(595, 190, self.get_turn_label(), 20, self.board.WHITE)

            """Add score label"""
            self.board.add_label(297, 283, str(self.player_b.score), 20, self.board.BLUE)
            self.board.add_label(297, 307, str(self.player_g.score), 20, self.board.GREEN)
            self.board.add_label(285, 295, str(self.player_y.score), 20, self.board.YELLOW)
            self.board.add_label(309, 295, str(self.player_r.score), 20, self.board.RED)
            ...
            if (player == "dice" and self.player == next_player
                and pygame.mouse.get_pressed()[0] and self.dice_rect.collidepoint(x, y)):
                self.roll_dice()
    ...
    def piece_starts(self, player, piece):
        ...
        throw = self.throw_out_piece(player, self.get_next_board_pos(piece, player.color))

        if throw != []:
            self.remove_img(throw[0][1].rect, self.board.get_pos_color(throw[0][1].rect))
    ...
    def throw_out_piece(self, player, pos):
        ...
        for other_player in self.player_list:
            if other_player != player:
                result = [(other_player, p) for p in other_player.pieces if pos == p.board_pos]
                if result != []:
                    return result
        return []
    ...
    def decide_move(self, player):
        ...
        front_list = [p.close_in_front for p in player.pieces if p.close_in_front != 0]
        back_list = [p.close_behind for p in player.pieces if p.close_behind != 0]
        min_front = 56
        min_behind = 56
        ...
        for piece in player.pieces:
            ...
            for other_player in self.player_list:
                if other_player != player:
                    ...
                    min_dist = self.min_behind_dist(player, piece, other_player)
    ...
    def make_move(self, player, rect):
        ...
        if self.board.go_left(rect):
            ...
            if ((rect.x > 300 - self.board.IMG_PLACE and player.color == 3) or
                ...
                rect.x -= 32 
            elif ((rect.x > 300 - self.board.IMG_PLACE and player.color != 3) or
                  (rect.y < 300 - self.board.IMG_PLACE and player.color == 1)):
                rect.y += 32
            elif ((rect.y < 300 - self.board.IMG_PLACE and player.color != 1) or
                  (rect.x < 300 - self.board.IMG_PLACE and player.color == 0)):
                rect.x += 32
            elif ((rect.x < 300 - self.board.IMG_PLACE and player.color != 0) or
                  (rect.y > 300 - self.board.IMG_PLACE and player.color == 2)):
                rect.y -= 32
    ...
    def get_free_place(self, player):
        ...
        for i in range(4):
            rect = pygame.Rect(
                (self.board.OUT[player.color][0] + self.board.PLACES[i][0] - self.board.IMG_PLACE,
                self.board.OUT[player.color][1] + self.board.PLACES[i][1] - self.board.IMG_PLACE),
    ...
    def remove_img(self, rect, color):
        pygame.draw.circle(self.screen, color, (rect.x + self.board.IMG_PLACE, rect.y + self.board.IMG_PLACE), 15)
        pygame.draw.circle(self.screen, self.board.BLACK, (rect.x + self.board.IMG_PLACE, rect.y + self.board.IMG_PLACE), 15, 1)

    def is_point_out(self, x, y):
        for i in range(4):
            if math.sqrt((self.board.OUT[i][0] - x) ** 2 + (self.board.OUT[i][1] - y) ** 2) <= 80:
                return True
        return False
    ...
    def change_turn(self):
        ...
        if player != "dice" and (self.dice != 6 or self.cant_move_count(player) == 4): 
            self.turn = (self.turn + 1) % 8
        if player != "dice" and self.dice == 6 and self.cant_move_count(player) != 4:
    ...
    def can_move(self, player, piece):
        ...
        if ((piece.pos + self.dice > 62) or
            ...
            (piece.pos != 0 and piece.pos + self.dice in [p.pos for p in player.pieces if p.pos != 62]) or
            (piece.pos == 0 and self.dice == 6 and 1 in [p.pos for p in player.pieces])):
            return False
        return True
